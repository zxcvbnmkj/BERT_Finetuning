...
单卡执行（第二个卡）：CUDA_VISIBLE_DEVICES=1 torchrun --nproc_per_node=1 train_DDP.py
"""
...
from utils import eval_classification, load_files, set_logger, tokenizering
...
def gather_metrics(train_true_labels, train_predictions):
    ...
    # 收集所有进程的数据到主进程，dst 指定哪个进程接收数据
    dist.gather(gathered_true_labels, true_labels_tensor, dst=0)
    dist.gather(gathered_predictions, predictions_tensor, dst=0)
    all_true_labels_global = torch.cat(gathered_true_labels).cpu().numpy()
    all_predictions_global = torch.cat(gathered_predictions).cpu().numpy()
...
def finetuning(epochs, max_patient, threshold, model_type, type):
    ...
    for epoch_i in trange(epochs, desc='Epoch'):
        ...
        for _, batch in train_dataloader:
            ...
            model.zero_grad()
            outputs = model(input_ids=b_input_ids, attention_mask=b_input_mask, token_type_ids=b_type, labels=b_labels)
        ...
        if world_size > 1:
            if is_main_process:
                all_true_labels_global, all_predictions_global = gather_metrics(train_true_labels, train_predictions)
                logging.info(f'\nEpoch {epoch_i + 1}/{epochs}')
                logging.info(f'Train loss: {avg_train_loss:.4f}')
                eval_classification(
                    pd.Series(all_true_labels_global),
                    pd.Series(all_predictions_global),
                    f'训练集_轮{epoch_i}',
                    use_log=True,
                )
        else:
            eval_classification(
                pd.Series(train_true_labels), pd.Series(train_predictions), f'训练集_轮{epoch_i}', use_log=True
            )
        ...
        with torch.no_grad():
            for batch in val_dataloader:
                b_input_ids, b_input_mask, b_type, b_labels = tuple(t.to(device) for t in batch)
                outputs = model(input_ids=b_input_ids, attention_mask=b_input_mask, token_type_ids=b_type)
                if model_type == 'official':
                    loss = outputs.loss
                    logits = outputs.logits.detach().cpu().numpy()
                elif model_type == 'custom':
        ...
        if world_size > 1:
            if is_main_process:
                val_all_true_labels_global, val_all_predictions_global = gather_metrics(
                    val_true_labels, val_predictions
                )
                report = eval_classification(
                    pd.Series(val_all_true_labels_global),
            ...
            report = eval_classification(
                pd.Series(val_true_labels), pd.Series(val_predictions), f'验证集_轮{epoch_i}', use_log=True
            )
        if is_main_process:
        ...
        if world_size > 1:
            ...
            # 所有进程 patient 的最大值
            dist.all_reduce(patient_tensor, op=dist.ReduceOp.MAX)
            patient = patient_tensor.item()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, default=40, help='单卡上的批次')
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='如果不为 0 表示使用子数据集训练与验证，将从原数据集中截取 sub_num 条数据',
    )
    parser.add_argument('--task', type=int, default=0, help='0: 单句任务；1: 双句任务，增加一个两个句子拼接的处理')
    parser.add_argument('--log_name', type=str, default='anwser_only', help='日志文件名字')
    parser.add_argument(
        '--model',
        type=str,
        default='official',
        help='模型形式：custom（自定义结构）、official（Transform库提供的官方结构）',
    )
    ...
    if df_valid is None:
        ...
        train_inputs, val_inputs, train_masks, val_masks, train_type, val_type, train_labels, val_labels = (
            train_test_split(
        ...
        train_sample = TensorDataset(
            encoded_inputs['input_ids'],
            encoded_inputs['attention_mask'],
            encoded_inputs['token_type_ids'],
            torch.tensor(train_labels),
        )
        val_sample = TensorDataset(
            encoded_inputs_valid['input_ids'],
            encoded_inputs_valid['attention_mask'],
            encoded_inputs_valid['token_type_ids'],
    ...
    if args.model == 'official':
        model = BertForSequenceClassification.from_pretrained(
            f'{dir_name}/chinese-bert-wwm', num_labels=2, classifier_dropout=0.5
        ).to(device)
    elif args.model == 'custom':
        model = BertCustomClassification(f'{dir_name}/chinese-bert-wwm').to(device)
